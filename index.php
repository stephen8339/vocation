...
<form action="index.php" method="get">
    ...
    年假每年八月一日重置，所以查询的假期从2016.7.31开始，之前的和未通过审批的不提供查询，假期信息每月更新一次。
...
<table style='text-align-all: left' border='1'>
    ...
    if(empty($_GET['name'])){
        ...
        $user = $_GET['name'];
        $conn = connectDb();
        $mysql_result = mysqli_query($conn,"SELECT * FROM details WHERE status = '完成' AND name = '$user' AND stime >= '$querydate'");
